...
from QRTS_Solvers import QRTS_PD
...
def main():
    ...
    parser.add_argument(
        '--trainNum', type=int, default=30, help='number of training data')
    
    parser.add_argument(
        '--testNum', type=int, default=30, help='number of testing data')
    ...
    parser.add_argument(
        '--max_iter_inner', type=int, default=1, help='if store a pre_train model')
    ...
    max_iter_inner = args.max_iter_inner
    #alpha = 0.618
    ...
    featurePath = "{}/{}/features/{}_{}".format(data_path, graph, featureGenMethod, eNum)
    ...
    for x, y in zip (X_train, Y_train):
        ...
        print()
    '''  
    P_realizations, realizationIndexes = source_USCO.readRealizations(featurePath, eNum, realizationRandom = False)
    ...
    qrts_PD = QRTS_PD()
    qrts_PD.initialize(featureNum, P_realizations, source_USCO)
        #**************************OneSlackSSVM_normal
    trainMethod = "one_slack_normal"
    w, D_realizations =qrts_PD.train(trainMethod, verbose, C, tol, thread, max_iter_inner, logpath,TrainQueries, TrainDecisions, beta, max_iter)
    ...
    Utils.writeToFile(logpath, "QRTS_PD featureNum: {}, featureGenMethod: {}, trainMethod: {}, beta: {}".format(featureNum, featureGenMethod, trainMethod, beta), toconsole = True,preTrainPathResult = preTrainPathResult)
    ...
    for TestSamples, TestQueries, TestDecisions in zip(TestSamples_s, TestQueries_s, TestDecisions_s):
        ...
        predDecisions = target_USCO.solve_R_batch(TestQueries, w, D_realizations, n_jobs=thread, offset=None, trace = False)
    ...
    #     randPredDecisions = target_USCO.solve_R_batch(TestQueries, randomW, D_realizations, n_jobs=thread, offset=None, trace=False)
